...
export class NewComponent {
  ...
  transformer = (node: TodoItemNode, level: number) => {
    ...
    this.nestedNodeMap.set(node, flatNode);
     console.log(flatNode.item)
    this.rightArr.push(flatNode.item);
  ...
  todoItemSelectionToggle(node: TodoItemFlatNode): void {
    ...
    this.selectedNodes = this.checklistSelection.selected.map(s => s);
    console.log(this.selectedNodes);
    console.log(node)
    console.log(this.checklistSelection.isSelected(node))
    this.flag = this.checklistSelection.isSelected(node);
    console.log(this.flag)
    // this.rightArr = this.checklistSelection.selected.map(s => s.item);
    console.log(this.rightArr)
  ...
  todoLeafItemSelectionToggle(node: TodoItemFlatNode): void {
    ...
    // this.rightArr = this.checklistSelection.selected.map(s => s.item);
    console.log(this.rightArr)
    // for (const arr of this.selectedNodes){
    //   if (arr.level === 0 ){
    //     const mcValue = arr;
    //     this.selectedNodes.splice(this.selectedNodes.length - 1, 1);
    //     this.selectedNodes.unshift(mcValue);
    // }
    // }
    // console.log(node);
    // this.flatNodeToTree(this.selectedNodes)
  }
  ...
  checkAll(){
    for (let i = 0; i < this.treeControl.dataNodes.length; i++) {
        this.checkRootNodeSelection(this.treeControl.dataNodes[i]);
